...
export default function FinanceManagementPage() {
  ...
  useEffect(() => {
    const fetchWalletData = async () => {
      try {
        ...
        const walletResponse = await axios.get(`http://localhost:4000/api/shipper/${shipperId}/total-wallet`, {
          headers: {
        ...
        const shipperResponse = await axios.get(`http://localhost:4000/api/shippers-auth/${shipperId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
  ...
  const handleDepositSelect = (amount) => {
    if (amount < walletData.minDeposit) {
      setError(`Số tiền nạp tối thiểu là ${formatCurrency(walletData.minDeposit)}`);
      return;
    }


    if (amount > walletData.maxDeposit) {
      setError(`Số tiền nạp tối đa là ${formatCurrency(walletData.maxDeposit)}`);
      return;
    }
    setError(null);
  ...
  const handleDepositAmountChange = (e) => {
    ...
    if (value && (amount < walletData.minDeposit || amount > walletData.maxDeposit)) {
      if (amount < walletData.minDeposit) {
        setError(`Số tiền nạp tối thiểu là ${formatCurrency(walletData.minDeposit)}`);
      } else if (amount > walletData.maxDeposit) {
        setError(`Số tiền nạp tối đa là ${formatCurrency(walletData.maxDeposit)}`);
      }
    } else {
      setError(null);
  ...
  const fetchTransactionHistory = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:4000/api/shipper/${shipperId}/transaction-history`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
                      ...
                      <div className="FinancialManagement-form-group">
                        ...
                        <div className="FinancialManagement-amount-options">
                          {[50000, 100000, 200000, 500000, 1000000, 2000000, 3000000].map((amount) => (
                            <button
                              key={amount}
                              className={`FinancialManagement-amount-option
                              ${selectedDepositAmount === amount ? "selected" : ""}
                              ${(amount < walletData.minDeposit || amount > walletData.maxDeposit) ? "disabled" : ""}`}
                              onClick={() => handleDepositSelect(amount)}
                              disabled={amount < walletData.minDeposit || amount > walletData.maxDeposit}
                            >
                              {formatCurrency(amount)}
                              {(amount < walletData.minDeposit || amount > walletData.maxDeposit) && (
                                <span className="FinancialManagement-option-warning">
                                  {amount < walletData.minDeposit ? "Quá thấp" : "Quá cao"}
                                </span>
                              )}
                            </button>
                          ))}
                        ...
                        <div className="FinancialManagement-input-wrapper">
                          ...
                          <input
                            ...
                            disabled={loading}
                            min={walletData.minDeposit}
                            max={walletData.maxDeposit}
                          />
                        </div>
                      </div>
                      <button
