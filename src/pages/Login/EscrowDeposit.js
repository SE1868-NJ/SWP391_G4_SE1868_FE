...
export default function EscrowDeposit() {
  ...
  const handleDepositSelect = (amount) => {
    if (amount < escrowData.minDeposit) {
      setError(`Số tiền nạp tối thiểu là ${formatCurrency(escrowData.minDeposit)}`);
      return;
    }
  ...
  const handleDepositAmountChange = (e) => {
    ...
    if (value) {
      if (amount < escrowData.minDeposit) {
        setError(`Số tiền nạp tối thiểu là ${formatCurrency(escrowData.minDeposit)}`);
      } else if (amount > escrowData.maxDeposit) {
        setError(`Số tiền nạp tối đa là ${formatCurrency(escrowData.maxDeposit)}`);
      } else {
        setError(null);
      }
  ...
  const handleDeposit = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:4000/api/escrow/deposit",
  ...
  const updateShipperStatus = async () => {
    if (escrowData.escrowBalance >= 1000000) {
      try {
        const response = await axios.post(
          "http://localhost:4000/api/shipper/update-status",
          { status: "Active" },
          {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          }
              ...
              <div className="EscrowDeposit-main-content">
                ...
                {showDepositSuccess ? (
                  <div className="EscrowDeposit-success-message">
                    ...
                    <p>
                      Số tiền {formatCurrency(Number(depositAmount))} đã được nạp vào ví ký quỹ.
                      {escrowData.escrowBalance >= 1000000
                        ? " Bạn đã đủ điều kiện để hoạt động."
                        : " Vui lòng nạp thêm để đủ điều kiện hoạt động!"}
                    </p>
                    <button
                    ...
                    <div className="EscrowDeposit-form-group">
                          ...
                          (amount) => (
                            <button
                              ...
                              className={`EscrowDeposit-deposit-option ${
                                ...
                                amount < escrowData.minDeposit || amount > escrowData.maxDeposit
                                  ? "disabled"
                                  : ""
                              }`}
                            ...
                            >
                              ...
                              {(amount < escrowData.minDeposit || amount > escrowData.maxDeposit) && (
                                <span className="EscrowDeposit-option-warning">
                                  {amount < escrowData.minDeposit ? "Quá thấp" : "Quá cao"}
                                </span>
                              )}
                            </button>
                          )
                      ...
                      <div className="EscrowDeposit-input-wrapper">
                        ...
                        <input
                          ...
                          disabled={loading}
                          min={escrowData.minDeposit}
                          max={escrowData.maxDeposit}
                        />
                      </div>
                      <div className="EscrowDeposit-limit-info">
                        <p>Tối thiểu: {formatCurrency(escrowData.minDeposit)}</p>
                        <p>Tối đa: {formatCurrency(escrowData.maxDeposit)}</p>
                      </div>
                    </div>
                    <button
                      ...
                      disabled={
                        !depositAmount ||
                        Number(depositAmount) < escrowData.minDeposit ||
                        Number(depositAmount) > escrowData.maxDeposit ||
                        loading
                      }
                    >
                      Nạp tiền
              ...
              <div className="EscrowDeposit-account-summary">
                ...
                <div>
                  <div className="EscrowDeposit-summary-item">
                    <span className="EscrowDeposit-summary-label">Số dư hiện tại</span>
                  ...
                  {remainingAmount > 0 && (
                    <div className="EscrowDeposit-summary-item">
                      <span className="EscrowDeposit-summary-label">Số tiền còn thiếu</span>
                      <span className="EscrowDeposit-summary-value">
                ...
                {escrowData.escrowBalance < 1000000 ? (
                  <p className="EscrowDeposit-warning">
                    Vui lòng nạp thêm ít nhất {formatCurrency(remainingAmount)} để đủ điều kiện hoạt động!
                  </p>
                ) : (
