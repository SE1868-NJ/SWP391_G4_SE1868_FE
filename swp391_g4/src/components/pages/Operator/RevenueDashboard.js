...
function SearchBar({ onSearch }) {
  ...
  return (
    <div className="RevenueDashboard-search-container">
  ...
  useEffect(() => {
    ...
    if (data && revenueByDay && revenueByDay.length > 0) {
      ...
      if (revenueByDay.length >= 7) {
        ...
        const firstHalfAvg =
          firstHalf.reduce((sum, day) => sum + day.revenue, 0) /
          firstHalf.length;
        const secondHalfAvg =
          secondHalf.reduce((sum, day) => sum + day.revenue, 0) /
          secondHalf.length;
        ...
        if (firstHalfAvg > 0) {
          ...
          if (percentChange < -20) {
            dynamicAlerts.push({
              type: "error",
              message: `Doanh thu có xu hướng giảm mạnh (${percentChange.toFixed(
                1
              )}%) trong tuần qua. Kiểm tra các yếu tố ảnh hưởng đến hoạt động kinh doanh.`,
              source: "analysis",
            });
          } else if (percentChange < -10) {
            dynamicAlerts.push({
              type: "warning",
              message: `Doanh thu có xu hướng giảm nhẹ (${percentChange.toFixed(
                1
              )}%) trong tuần qua. Theo dõi tình hình.`,
              source: "analysis",
            });
          } else if (percentChange > 20) {
            dynamicAlerts.push({
              type: "success",
              message: `Doanh thu có xu hướng tăng mạnh (${percentChange.toFixed(
                1
              )}%) trong tuần qua. Duy trì chiến lược hiện tại.`,
              source: "analysis",
            });
          }
        }
      ...
      const avgRevenue =
        recentDaysData.reduce((sum, day) => sum + day.revenue, 0) /
        recentDaysData.length;
      ...
      if (mostRecentDay && avgRevenue > 0) {
        const revenueChange =
          ((mostRecentDay.revenue - avgRevenue) / avgRevenue) * 100;

        // Chỉ cảnh báo khi có sự chênh lệch lớn
        if (revenueChange < -30) {
          dynamicAlerts.push({
            type: "warning",
            message: `Doanh thu ngày ${mostRecentDay.date} giảm ${Math.abs(
              revenueChange
            ).toFixed(
              1
            )}% so với trung bình ${daysToAnalyze} ngày (${formatCurrency(
              mostRecentDay.revenue
            )} so với ${formatCurrency(avgRevenue)}).`,
            source: "analysis",
          });
        } else if (revenueChange > 50) {
          dynamicAlerts.push({
            type: "info",
            message: `Doanh thu ngày ${
              mostRecentDay.date
            } tăng ${revenueChange.toFixed(
              1
            )}% so với trung bình. Kiểm tra sự kiện đặc biệt hoặc xác nhận tính chính xác của dữ liệu.`,
            source: "analysis",
          });
        }
      }
      ...
      if (revenueByRegion) {
        ...
        if (totalRegionRevenue > 5000000) {
          ...
          if (centralPercent < idealDistribution.central * 0.5) {
            dynamicAlerts.push({
              type: "warning",
              message: `Doanh thu Khu vực Trung tâm thấp bất thường (${(
                centralPercent * 100
              ).toFixed(1)}% so với mục tiêu ${(
                idealDistribution.central * 100
              ).toFixed(1)}%). Kiểm tra hoạt động.`,
              source: "analysis",
            });
          }
          ...
          if (midZonePercent < idealDistribution.midZone * 0.5) {
            dynamicAlerts.push({
              type: "warning",
              message: `Doanh thu Khu vực Quanh trung tâm thấp bất thường (${(
                midZonePercent * 100
              ).toFixed(1)}% so với mục tiêu ${(
                idealDistribution.midZone * 100
              ).toFixed(1)}%). Kiểm tra hoạt động.`,
              source: "analysis",
            });
          }
          ...
          if (outerZonePercent < idealDistribution.outerZone * 0.5) {
            dynamicAlerts.push({
              type: "warning",
              message: `Doanh thu Khu vực Rìa trung tâm thấp bất thường (${(
                outerZonePercent * 100
              ).toFixed(1)}% so với mục tiêu ${(
                idealDistribution.outerZone * 100
              ).toFixed(1)}%). Kiểm tra hoạt động.`,
              source: "analysis",
            });
          }
      ...
      if (revenueByService) {
        const normalizedService = {
          standard: Number(
            revenueByService.standard || revenueByService.Standard || 0
          ),
          express: Number(
            revenueByService.express || revenueByService.Express || 0
          ),
          scheduled: Number(
            revenueByService.scheduled || revenueByService.Scheduled || 0
          ),
        };
        ...
        const totalServiceRevenue =
          ...
          normalizedService.scheduled;

        // Chỉ phân tích khi có doanh thu đáng kể
        if (totalServiceRevenue > 10000000) {
          // Các chỉ số KPI tham chiếu
          const expressTarget = 0.25; // Mục tiêu tỷ lệ dịch vụ express
          const scheduledTarget = 0.2; // Mục tiêu tỷ lệ dịch vụ scheduled

          const expressPercent =
            normalizedService.express / totalServiceRevenue;
          const scheduledPercent =
            normalizedService.scheduled / totalServiceRevenue;

          // So sánh với mục tiêu
          if (expressPercent < expressTarget * 0.7) {
            dynamicAlerts.push({
              type: "info",
              message: `Dịch vụ giao hàng nhanh đạt ${(
                expressPercent * 100
              ).toFixed(1)}% (mục tiêu: ${(expressTarget * 100).toFixed(
                1
              )}%). Cân nhắc xem xét chính sách giá và marketing.`,
              source: "analysis",
            });
          }
          ...
          if (scheduledPercent > scheduledTarget * 1.5) {
            dynamicAlerts.push({
              type: "success",
              message: `Dịch vụ giao hàng hẹn giờ đang vượt mục tiêu (${(
                scheduledPercent * 100
              ).toFixed(1)}% so với ${(scheduledTarget * 100).toFixed(
                1
              )}%). Xem xét mở rộng năng lực.`,
              source: "analysis",
            });
          }
        }
      ...
      if (data.revenue) {
        ...
        const projectedMonthlyRevenue =
          (currentRevenue / currentDayOfMonth) * daysInMonth;

        // Chỉ cảnh báo khi có nguy cơ không đạt mục tiêu
        if (projectedMonthlyRevenue < monthlyTarget * 0.85) {
          dynamicAlerts.push({
            type: "error",
            message: `Dự báo doanh thu tháng (${formatCurrency(
              projectedMonthlyRevenue
            )}) thấp hơn ${(
              (1 - projectedMonthlyRevenue / monthlyTarget) *
              100
            ).toFixed(1)}% so với mục tiêu. Cần có biện pháp khẩn cấp.`,
            source: "analysis",
          });
          ...
          dynamicAlerts.push({
            ...
            message: `Dự báo doanh thu tháng (${formatCurrency(
              projectedMonthlyRevenue
            )}) có nguy cơ không đạt mục tiêu. Xem xét các biện pháp cải thiện.`,
            source: "analysis",
          });
        }
      }
  ...
  return (
    <div className="RevenueDashboard-alerts-container">
      {generatedAlerts.map((alert, index) => (
        <div
          key={index}
          className={`RevenueDashboard-alert RevenueDashboard-alert-${alert.type}`}
        >
          ...
          <button
            className="RevenueDashboard-alert-close"
            onClick={() => {
    ...
    <div className="RevenueDashboard-panel">
      ...
      <div className="RevenueDashboard-stats-grid">
        <div className="RevenueDashboard-stat-card">
          <div className="RevenueDashboard-stat-value">
        ...
        <div className="RevenueDashboard-stat-card">
          ...
          <div className="RevenueDashboard-stat-label">
            Doanh thu trung bình/đơn
          </div>
        </div>
        <div className="RevenueDashboard-stat-card">
          <div className="RevenueDashboard-stat-value">
            {formatCurrency(data.profitAfterFees)}
          </div>
...
function OrdersPanel({ filters }) {
  ...
  const handlePageChange = (page) => {
    ...
    const tableElement = document.querySelector(
      ".RevenueDashboard-table-responsive"
    );
    if (tableElement) {
  ...
  return (
    <div className="RevenueDashboard-panel">
      <div className="RevenueDashboard-panel-header">
        <div className="RevenueDashboard-panel-title">
          ...
          {totalOrders > 0 && (
            <div className="RevenueDashboard-paginator">
              <div className="RevenueDashboard-paginator-info">
                Hiển thị{" "}
                {totalOrders === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1}-
                {Math.min(currentPage * itemsPerPage, totalOrders)} trong số{" "}
...
function RegionRevenuePanel({ revenueByRegion }) {
  ...
  const safeRevenueByRegion = revenueByRegion || {
    central: 0,
    mid_zone: 0,
    outer_zone: 0,
  };
  ...
  const data = [
    ...
    {
      name: "Quanh trung tâm",
      value: Number(safeRevenueByRegion.mid_zone) || 0,
    },
    {
      name: "Rìa trung tâm",
      value: Number(safeRevenueByRegion.outer_zone) || 0,
    },
  ];
          ...
          <tbody>
            {payments.map((payment) => (
              <tr key={payment.id}>
                ...
                <td>
                  <span className="RevenueDashboard-status status-success">
                    {payment.status}
                  </span>
                </td>
              </tr>
    ...
    const fetchAllData = async () => {
      try {
        ...
        const [
          ...
          axios.get("http://localhost:5000/api/revenue-overview", {
            params: apiParams,
          }),
          axios.get("http://localhost:5000/api/revenue-by-day", {
            params: apiParams,
          }),
          axios.get("http://localhost:5000/api/revenue-by-region", {
            params: apiParams,
          }),
          axios.get("http://localhost:5000/api/revenue-by-service", {
            params: apiParams,
          }),
        ...
        const revenueByRegion = revenueByRegionRes.data || {
          central: 0,
          mid_zone: 0,
          outer_zone: 0,
        };
        const revenueByService = revenueByServiceRes.data || {
          standard: 0,
          express: 0,
          scheduled: 0,
        };
