...
const OrderDetails = () => {
  ...
  const fetchShipperBalance = async (shipperID) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/getShipperBalance/${shipperID}`,
        {
  ...
  useEffect(() => {
    const updateShippingFee = async () => {
      if (distance > 0 && orderData.order.OrderID) {
        ...
        try {
          await axios.put(`http://localhost:5000/api/updateShippingFee`, {
  ...
  const pickOrder = async () => {
    try {
      ...
      await axios.put('http://localhost:5000/api/pickOrder', pickOrderData);
  ...
  const confirmOrder = async (status) => {
    try {
      if (status === 'Cancelled') {
        ...
        const response = await axios.put('http://localhost:5000/api/confirm-delivery-order', {
          OrderID: id,
          Status: status
        });
  
        // Lấy lại thông tin đơn hàng để tính toán
        const orderResponse = await axios.get(`http://localhost:5000/api/getOrderDetails/${id}`);
