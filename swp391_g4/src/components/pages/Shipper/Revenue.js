...
const Revenue = () => {
    ...
    const fetchAllOrders = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/get-all-my-delivery-orders/" + id
        );
  ...
  useEffect(() => {
    const newTotalRevenue = orders.reduce(
      (acc, order) => acc + Number(order.ShippingFee),
      0
    );
    ...
    <div>
      ...
      <div className="container d-block">
        ...
        <div className="d-flex w-100">
          <div
            className="shadow-sm rounded-3 w-75 me-4"
            style={{ backgroundColor: "#cdf4e1" }}
          >
            <div className="p-4">
