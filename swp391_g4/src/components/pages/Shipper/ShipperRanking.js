...
import { FaTrophy, FaMedal, FaStar, FaShippingFast, FaChartLine, FaQuestionCircle, FaChevronLeft, FaChevronRight, FaCheckCircle, FaStarHalfAlt } from 'react-icons/fa';
...
const ShipperRanking = () => {
  ...
  const fetchRankings = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:4000/api/shippers/rankings/calculate`, {
        params: {
          month: selectedMonth
        },
        headers: {
      ...
      const myRank = response.data.find(shipper => shipper.ShipperID === parseInt(shipperId));
      setMyRanking(myRank);
    ...
    <div className="shipper-ranking">
      ...
      <div className="container mt-5">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 className="ranking-title w-100 text-center">Bảng Xếp Hạng Shipper</h2>
          <button 
            ...
            <div className="my-ranking-content">
              <div className="my-ranking-info">
                ...
                <div className="d-flex align-items-center">
                  {getRankIcon(rankings.findIndex(s => s.ShipperID === myRanking.ShipperID) + 1)}
                  <span className="ms-2">Hạng {rankings.findIndex(s => s.ShipperID === myRanking.ShipperID) + 1}</span>
                </div>
                <div className="my-ranking-stats">
                  <div className="stat-item">
                    <FaShippingFast className="stat-icon" />
                    <span>{myRanking.SuccessfulOrders || 0} đơn thành công</span>
                  </div>
                  <div className="stat-item">
                      ...
                      <table className="table table-hover">
                        ...
                        <tbody>
                          {currentItems.map((shipper, index) => (
                            <tr key={shipper.ShipperID} className={shipper.ShipperID === myRanking?.ShipperID ? 'my-rank-row' : ''}>
                              <td>
                                <div className="d-flex align-items-center">
                                  {getRankIcon(indexOfFirstItem + index + 1)}
                                  <span className="ms-2">{indexOfFirstItem + index + 1}</span>
                                </div>
                              </td>
          ...
          <div className="rules-modal-content">
            ...
            <div className="rules-modal-body">
                ...
                {[1, 2, 3, 4].map((rank) => {
                  ...
                  return (
                    <div key={rank} className="prize-rule-item">
                      ...
                      <p className="prize-rule-description">{prize.description}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <ul>
                ...
                <li>Điểm đánh giá được tính dựa trên đánh giá của khách hàng</li>
